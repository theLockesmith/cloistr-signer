...
func (ps *PostgresStorage) migrate() error {
	...
	-- Add owner_id for multi-user key isolation (SECURITY FIX)
	...
	-- Add cover_traffic column for presence-masking decoys (Phase: Privacy Architecture §3.11)
	ALTER TABLE signer_keys ADD COLUMN IF NOT EXISTS cover_traffic BOOLEAN NOT NULL DEFAULT FALSE;
	...
	-- Grant takeover audit: when a new client connects to a role key, the old
	-- grant is displaced (not deleted). revoked_at records the moment, revoked_by
	-- records who displaced it. A natural expiry has revoked_at NULL and
	-- expires_at in the past; a displacement has both filled.
	ALTER TABLE signer_permissions ADD COLUMN IF NOT EXISTS revoked_at TIMESTAMPTZ;
	ALTER TABLE signer_permissions ADD COLUMN IF NOT EXISTS revoked_by TEXT;
	...
	FROM ranked
	WHERE signer_permissions.key_id = ranked.key_id
...
func (ps *PostgresStorage) UpdateKey(ctx context.Context, key *Key) error {
	result, err := ps.db.ExecContext(ctx, `
		UPDATE signer_keys SET name = $1, require_approval = $2, disposable_mode = $3, cover_traffic = $4, tor_egress = $5, relays = $6,
			key_type = $7, bunker_uri = $8, upstream_pubkey = $9
		WHERE id = $10`,
		key.Name, key.RequireApproval, key.DisposableMode, key.CoverTraffic, key.TorEgress, pq.Array(key.Relays),
