...
func (ss *SQLiteStorage) initSchema() error {
	...
	-- Recovery materials (encrypted_user_share_at_dkg + user_verification_share)
	-- are present from the table definition forward; on older sqlite-backed
	-- dev DBs the columns are absent and existing FROST keys can't be
	-- recovered (sqlite is dev/test only; postgres handles existing rows
	-- via ALTER TABLE IF NOT EXISTS in postgres.go).
	CREATE TABLE IF NOT EXISTS signer_frost_user_shares (
		...
		created_at                  TEXT NOT NULL DEFAULT (datetime('now')),
		updated_at                  TEXT NOT NULL DEFAULT (datetime('now')),
		encrypted_user_share_at_dkg BLOB,
...
func (ss *SQLiteStorage) CreateKey(ctx context.Context, key *Key) error {
	...
	_, err := ss.db.ExecContext(ctx, `
		INSERT INTO signer_keys (id, name, pubkey, key_type, encrypted_nsec, encryption_method, bunker_uri, upstream_pubkey, require_approval, disposable_mode, cover_traffic, tor_egress, relays, created_at, created_by, owner_id)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		boolToInt(key.DisposableMode), boolToInt(key.CoverTraffic), boolToInt(key.TorEgress),
...
func (ss *SQLiteStorage) UpdateKey(ctx context.Context, key *Key) error {
	result, err := ss.db.ExecContext(ctx, `
		UPDATE signer_keys SET name = ?, require_approval = ?, disposable_mode = ?, cover_traffic = ?, tor_egress = ?, relays = ?,
			key_type = ?, bunker_uri = ?, upstream_pubkey = ?
		WHERE id = ?`,
		nullStr(key.Name), boolToInt(key.RequireApproval), boolToInt(key.DisposableMode), boolToInt(key.CoverTraffic), boolToInt(key.TorEgress), stringsToJSONArray(key.Relays),
...
func (ss *SQLiteStorage) CreateFrostUserShare(ctx context.Context, share *FrostUserShare) error {
	...
	_, err := ss.db.ExecContext(ctx, `
		...
		share.EncryptedUserShareAtDkg, nullStr(share.UserVerificationShareHex))
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
...
func (ss *SQLiteStorage) UpdateFrostUserShare(ctx context.Context, share *FrostUserShare) error {
	...
	result, err := ss.db.ExecContext(ctx, `
		...
		nullStr(share.UserVerificationShareHex),
		share.ID)
	if err != nil {
		return err
	}
